...
class _Froe(object):
    def __init__(self, args): #init all parameters from terminal or default values
        ...
        self.nU = args.nu if args.nu != None else 7
        self.nY= args.ny if args.ny != None else 5
        self.polDegree = args.deg if args.deg != None else 2
        self.inclBias = args.incbias if args.incbias != None else True
        self.varianceToExplain1 = args.var1 if args.var1 != None else 0.70
        self.varianceToExplain2 = args.var2 if args.var2 != None else 0.912
        self.convergenceThresholdNARMAX = args.convth if args.convth != None else 0.05
        self.maxIterationsNARMAX = args.maxit if args.maxit != None else 2
    ...
    def calcTheta(self, G, A, regressorToKeep, regDim):
    ...
    def calcSRR(self,i,j,A,G,W,regressorToKeep,w,g,msse,U,Y,nU,nY,nE,yDim,regDim,polDegree,inclBias,sum_y_pow_2,Phi,actualMSSE,K):
        wi = Phi[:,i].copy()
        if i not in regressorToKeep:
            ...
            if (K == 0 or( K != 0 and YhatPred is not None)):
                ...
                if (YhatSim is not None):
                    ...
                    msse[i] = MSSEi
                    return  (actualMSSE - MSSEi) / (sum_y_pow_2 / yDim), wi, msse[i] , g[i]
                else:
                    return  -999999.0 , wi, msse[i] ,g[i]
            else:
                return  -999999.0 , wi, msse[i], g[i]
        else:
            return -999999.0, wi, msse[i], g[i]
    ...
    def froe(self, nU, nY, nE, U, Y, E, polDegree, yDim, inclBias, varianceToExplain1, varianceToExplain2, K, th, maxIt): #froe algorithm
        ...
        for j in tqdm(range(regDim)): #calculates err, g, updates a
            w = np.zeros(shape=(yDim,regDim), dtype = np.float64)
            g = np.zeros(shape=(regDim), dtype = np.float64)
            msse = np.zeros(shape=(regDim), dtype = np.float64)
            srr = np.full(shape=(regDim),fill_value=-999999.0, dtype=np.float64)

            temp = Parallel(n_jobs = 8)(delayed(self.calcSRR)(i,j,A,G,W,regressorToKeep,w,g.copy(),msse,U,Y,nU,nY,nE,yDim,regDim,polDegree,inclBias,sum_y_pow_2,Phi,actualMSSE,K) for i in range(regDim))

            for u in range(regDim):
                srr[u] = temp[u][0]
                w[:,u] = temp[u][1]
                msse[u] = temp[u][2]
                g[u] = temp[u][3]
            ...
            if (np.all(srr <= 0) and (j != 0)):
                if (nE != 0):
                    print("Iteration",K)
                print("taken" , len(regressorToKeep) , "/" , regDim , "parameters explaining" , varianceExplained, "variance\n")
                break

            bestRegressor=np.argmax(srr)
            regressorToKeep = np.append(regressorToKeep, bestRegressor)

            varianceExplained += 1 + srr[bestRegressor] if (j == 0) else srr[bestRegressor]
            actualMSSE = msse[bestRegressor]
        ...
        Theta = self.calcTheta(G, A, regressorToKeep, regDim)
        ...
        if (nE != 0): #next iterations when narmax
            if(K == 0):
                ...
                Yhat = self.generateYhat(0, U, Y, np.zeros(shape = (yDim), dtype = np.float64), Theta, polDegree, inclBias, yDim, nU, nY, nE, False)
            if (Yhat is None):
    ...
    def generateYhat(self, type, u, y, e, theta, polDegree, inclBias, yDim, nU, nY, nE, forIde): #generates Yhat
    ...
    def run(self): #main algorithm
        ...
        #Training Set
        U, Y = np.append(u11, u1) , np.append(z11, z1)
        ...
        YhatPredictionIde = self.generateYhat(0, U, Y, np.zeros(shape = (yDimIde), dtype = np.float64), ThetaSelected, polDegree, inclBias, yDimIde, nU, nY, nE, False)
        if (not YhatPredictionIde is None):
            ...
            YhatSimulationIde = self.generateYhat(1, U, Y, residualsPredictionIde, ThetaSelected, polDegree, inclBias, yDimIde, nU, nY, nE, False)
        ...
        YhatPredictionVal1 = self.generateYhat(0, Uval1, Yval1, np.zeros(shape = (yDimVal1), dtype = np.float64), ThetaSelected, polDegree, inclBias, yDimVal1, nU, nY, nE, False)
        if (not YhatPredictionVal1 is None):
            ...
            YhatSimulationVal1 = self.generateYhat(1, Uval1, Yval1, residualsPredictionVal1, ThetaSelected, polDegree, inclBias, yDimVal1, nU, nY, nE, False)
        ...
        YhatPredictionVal2 = self.generateYhat(0, Uval2, Yval2, np.zeros(shape = (yDimVal2), dtype = np.float64), ThetaSelected, polDegree, inclBias, yDimVal2, nU, nY, nE, False)
        if (not YhatPredictionVal2 is None):
            ...
            YhatSimulationVal2 = self.generateYhat(1, Uval2, Yval2, residualsPredictionVal2, ThetaSelected, polDegree, inclBias, yDimVal2, nU, nY, nE, False)
